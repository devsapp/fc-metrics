...
exports.getRpcName = exports.dateFormat = exports.test = exports.convertTreeOne = exports.convertTree = exports.convertRpcToTree = void 0;
const moment_1 = __importDefault(require("moment"));
// import { getRpcTypeInfoList } from "../services/traceServices";
/*
 * 通过rpcId把数组转换成tree型结构
 * */ const i18n_1 = __importDefault(require("./i18n"));
function convertRpcToTree(list) {
    let tree = [], endTime = 0, startTime = 0, minLevel = Infinity, len = list.length;
    // //修复集团内的情况
    ...
    if (len > 1) {
        ...
        list.sort(function (a, b) {
            // fixGroup(a);
            // fixGroup(b);
            let i = 0, m, n, aLevels = a.rpcId.split("."), bLevels = b.rpcId.split("."), len = Math.min(aLevels.length, bLevels.length);
            a.levels = aLevels;
            b.levels = bLevels;
            if (minLevel > len) {
                minLevel = len;
    ...
    list.forEach(function (d, i) {
        ...
        if ("logTime" in d && "span" in d) {
            let s = new Date(d.logTime).getTime(), e = s + d.span;
...
exports.convertRpcToTree = convertRpcToTree;
function convertTree(list) {
    let tree = [], endTime = 0, startTime = 0;
    ...
    list.forEach(function (d, i) {
        ...
        if ("Timestamp" in d && "Duration" in d) {
            let s = new Date(parseInt(d.Timestamp)).getTime(), e = s + d.Duration;
            if (!startTime || s < startTime) {
                startTime = s;
            }
            if (e > endTime) {
...
//有一个children的不缩进
function convertTreeOne(list) {
    let tree = [], endTime = 0, startTime = 0, formatTree = [], formatTreeParentIdData = [];
    function findTreeNode(arr) {
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].children && arr[i].children.length) {
                arr[i]._node = i + 1;
...
exports.convertTreeOne = convertTreeOne;
function test(tree, arr) {
...
function dateFormat(date, format) {
    ...
    return moment_1.default(newDate).format(format);
}
...
//   }
// });
function getRpcName(code) {
    let i = 0, l = rpcTypes.length;
    for (; i < l; i += 1) {
        if (rpcTypes[i].code === code) {
            return rpcTypes[i].name;
        }
    }
    return i18n_1.default.get({ id: "xtrace2.src.utils.dataConvert.Unknown", dm: "未知" });
}
