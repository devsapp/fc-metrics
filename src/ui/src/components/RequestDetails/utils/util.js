import $i18n from "../i18n"; // import request, { getRequest, postRequest } from '../utils/request';
 ...
 * @param {*} params
 */
export function i18nDocLinkFun(key, def) {
 ...
 * @param {*} params
 */
export const getMinMax = function(arr, key) {
  let newArr = arr.map(item => {
    let val = item[key];
    if (isNaN(val)) {
      val = 0;
    }
    return val.toFixed(getFixed(val)) - 0;
  });
...
//格式化响应时间
export const formatterRt = value => {
  let val = 0;
  if (!isNaN(value)) {
    if (value >= 3600000) {
      val = value / 3600000;
      return val.toFixed(getFixed(val)) + "h";
    } else if (value >= 60000) {
      val = value / 60000;
      return val.toFixed(getFixed(val)) + "min";
    } else if (value >= 1000) {
      val = value / 1000;
      return val.toFixed(getFixed(val)) + "s";
    } else {
      val = value - 0;
      return val.toFixed(getFixed(val)) + "ms";
    }
  }
  return value;
};
// 根据查询时间段格式化对应单位
export const formatMinuteYAxes = dimValue => {
  let val = 0;
  let time = "";
  if (!isNaN(dimValue)) {
    if (dimValue >= 86400) {
      val = dimValue / 86400;
      time = $i18n.get({ id: "xtrace2.src.utils.util.Day", dm: "日" });
    } else if (dimValue >= 3600) {
      val = dimValue / 3600;
      time = $i18n.get({ id: "xtrace2.src.utils.util.AnArcane", dm: "时" });
    } else if (dimValue >= 60) {
      val = dimValue / 60;
      time = $i18n.get({ id: "xtrace2.src.utils.util.Points", dm: "分" });
    } else if (dimValue >= 1) {
      val = dimValue / 1;
      time = $i18n.get({ id: "xtrace2.src.utils.util.Seconds", dm: "秒" });
    }
    if (val === 1) {
      val = "";
    }
...
// count单位转换
export const format1000YAxes = dimVal => {
  ...
  if (!isNaN(dimValue)) {
    if (dimValue >= 10000000000000) {
      ...
      return val.toFixed(getFixed(val)) + "T";
    } else if (dimValue >= 10000000000) {
      val = dimValue / 1000000000;
      return val.toFixed(getFixed(val)) + "G";
      
    } else if (dimValue >= 10000000) {
      val = dimValue / 1000000;
      return val.toFixed(getFixed(val)) + "M";
    } else if (dimValue >= 10000) {
      val = dimValue / 1000;
      return val.toFixed(getFixed(val)) + "K";
    } else if (dimValue < 0) {
      return 0;
    }
  }
...
export const formatMemYAxes = dimValue => {
  // 千用K是国际通用的,表示前缀Kilo
...
export const formatChartData = (data, name, y, x = "date", alias) => {
  if (!data || !data.length) {
    return;
  }
  let arr = [];
...
// 格式化图表数据,格式化为[{name:'响应时间',data:[[x,y]]}]这样的格式
//  data: 数组, name：图例名称, y：y轴, x: x轴, alias: {} 有时候处理国际化会用到
export const exceptionFormatChartData = (data, name, y, x = "date") => {
  if (!data || !data.length) {
    return;
  }
  let arr = [];
...
export const formatDimChartDateData = (ds = {}, options = {}) => {
  let dims = ds.dims;
...
export const isSameMoments = (a = [], b = []) => {
...
export const getArrKeySum = (data, ...keys) => {
...
export const createObj = (str, value) => {
...
export const sortData = (data, sortKey, sort) => {
  if (sort == "desc") {
...
export const getAppSiteType = pid => {
  const appList = window.APP_LIST || [];
  const app = appList.find(n => {
    return n.pid === pid;
  });
...
export function decoratorFactory(name = 'default') {
  if (!(name in DF)) {
    DF[name] = decorator()
  }
  return DF[name];
...
export function toCopy(ctx) {
  copy(ctx);
  Message.success('copy success');
}

export function strTemp(temp, data, options) {
...
export function intlTemp(key, data) {
  const str = (key in messages) ? messages[key] : key;
...
export const getHashMap = (data, primaryKeys = [], isFormatObj = false) => {
  let hashMap = {};
  if (!isFormatObj) {
    data.slice().map(item => {
      let primary = "";
